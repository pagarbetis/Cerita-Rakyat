...
if (this.#map) {
  const latitude = report.lat ?? report.location?.latitude;
  const longitude = report.lon ?? report.location?.longitude;
  ...
  if (
    ...
    console.warn('Lewati marker: koordinat tidak valid', report);
  }
}
      ...
      return accumulator.concat(
        generateReportItemTemplate({
          ...
          reporterName: report.reporter?.name ?? report.name ?? 'Tidak diketahui',
