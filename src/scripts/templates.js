...
export function generateReportDetailImageTemplate(photoUrl = null, alt = '') {
  if (!photoUrl) {
    return `
  ...
  return `
    <img class="report-detail__image" src="${photoUrl}" alt="${alt}">
  `;
}
...
export function generateReportDetailTemplate({
  ...
  const placeName = location?.placeName || 'Lokasi tidak diketahui';

  return `
    <div class="report-detail__header">
      <h1 id="title" class="report-detail__title">${name || 'Tanpa Judul'}</h1>
      ...
      <div class="report-detail__more-info">
        <div class="report-detail__more-info__inline">
          ...
          <div id="location-place-name" class="report-detail__location__place-name" data-value="${placeName}">
            <i class="fas fa-map"></i> ${placeName}
          </div>
        </div>
        <div class="report-detail__more-info__inline">
          <div id="location-latitude" class="report-detail__location__latitude" data-value="${lat}">
            Latitude: ${lat}
          </div>
          <div id="location-longitude" class="report-detail__location__longitude" data-value="${lon}">
            Longitude: ${lon}
          </div>
        </div>
      </div>
